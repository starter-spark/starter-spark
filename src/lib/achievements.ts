...
export async function awardAchievement(
  ...
  try {
    ...
    if (typeof data !== 'boolean') {
      return {
        success: false,
        error: 'Unexpected response from award_achievement',
      }
    }
...
async function getLessonContext(
  ...
  if (!lessonContext) {
    console.warn('Lesson not found while checking achievements', {
      lessonId,
      userId,
    })
    return null
...
async function checkLessonCountAchievements(
  ...
  const { count: totalCompletedCount, error: totalCountError } =
    await supabaseAdmin
...
async function checkSpeedLearnerAchievement(
  userId: string,
...
async function checkModuleCompleteAchievement(
  ...
  const { count: completedInModule, error: moduleCountError } =
    await supabaseAdmin
      .from('lesson_progress')
      .select('lesson_id', { count: 'exact', head: true })
      .eq('user_id', userId)
...
async function checkCourseCompleteAchievement(
  ...
  const { count: completedInCourse, error: courseCountError } =
    await supabaseAdmin
      .from('lesson_progress')
      .select('lesson_id', { count: 'exact', head: true })
      .eq('user_id', userId)
...
export async function checkLessonAchievements(
  ...
  const moduleComplete = await checkModuleCompleteAchievement(
    userId,
...
export async function checkKitClaimAchievements(
  userId: string,
): Promise<AwardResult[]> {
  const results: AwardResult[] = []
...
export async function getUserAchievements(
  ...
  const { data: achievementsData, error: achievementsError } = await supabase
    .from('achievements')
    .select(
      'id, key, name, description, icon, points, category, is_secret, sort_order, unlock_hint',
    )
    .order('sort_order')
