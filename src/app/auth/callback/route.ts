...
import { maskEmail, maskLicenseCode } from '@/lib/masks'
  ...
  if (code) {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL('/login?error=auth_failed', SITE_URL),
      )
    }
    ...
    if (isNewUser && user.email) {
      try {
        ...
        if (profileError) {
          console.error(
            'Failed to fetch profile name for welcome email:',
            profileError,
          )
        }
    ...
    if (claimToken && user && isValidClaimToken(claimToken)) {
      try {
        ...
        const { data: license, error: fetchError } = await supabaseAdmin
          .from('licenses')
          .select(
            'id, code, status, owner_id, customer_email, product:products(name)',
          )
          .eq('claim_token', claimToken)
          .maybeSingle()

        if (fetchError || !license) {
          // Token used or missing, continue redirect flow.
          ...
          const { data: claimedLicense, error: claimError } =
            await supabaseAdmin
              .from('licenses')
              .update({
                owner_id: user.id,
          ...
          if (claimError) {
            ...
            console.log(
              `License ${maskLicenseCode(claimedLicense.code, { mode: 'normalized' })} claimed by user ${user.id} (status=${claimedLicense.status})`,
            )
          }
        }
      } catch (err) {
