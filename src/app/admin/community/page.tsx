...
export default async function CommunityPage({
  ...
  const [posts, stats] = await Promise.all([getPosts(params.status), getPostStats()])
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="font-mono text-2xl font-bold text-slate-900">Community</h1>
        <p className="text-slate-600">Moderate posts and community discussions</p>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-4">
          ...
          <p className="font-mono text-2xl font-bold text-red-600">{stats.flagged}</p>
        </div>
      </div>
