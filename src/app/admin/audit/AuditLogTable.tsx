...
function DetailsSection({ details }: { details: Json | null }) {
  ...
  const hasOldNew = "old" in detailsObj || "new" in detailsObj ||
                    "oldRole" in detailsObj || "newRole" in detailsObj ||
                    "oldStatus" in detailsObj || "newStatus" in detailsObj

  if (hasOldNew) {
    const pairs: { label: string; old: string; new: string }[] = []

    if ("oldRole" in detailsObj && "newRole" in detailsObj) {
      pairs.push({ label: "Role", old: formatValue("oldRole", detailsObj.oldRole), new: formatValue("newRole", detailsObj.newRole) })
    }
    if ("oldStatus" in detailsObj && "newStatus" in detailsObj) {
      pairs.push({ label: "Status", old: formatValue("oldStatus", detailsObj.oldStatus), new: formatValue("newStatus", detailsObj.newStatus) })
    }
    if ("old" in detailsObj && "new" in detailsObj) {
      pairs.push({ label: "Value", old: formatValue("old", detailsObj.old), new: formatValue("new", detailsObj.new) })
    }
        ...
        <div className="space-y-2">
          {pairs.map((pair, i) => (
            <div key={i} className="flex items-center gap-2 text-sm">
              ...
              <code className="px-2 py-0.5 bg-red-50 text-red-700 rounded text-xs">{pair.old}</code>
              <ArrowRight className="h-3 w-3 text-slate-400" />
              <code className="px-2 py-0.5 bg-green-50 text-green-700 rounded text-xs">{pair.new}</code>
            </div>
          ))}
