...
export default async function AccountPage() {
  ...
  const { data: profile, error } = await supabase
    .from("profiles")
    .select("id, email, full_name, avatar_url, avatar_seed, role, is_banned_from_forums, ban_reason, created_at")
    .eq("id", user.id)
    .maybeSingle()
  ...
  return (
    <main className="min-h-screen bg-slate-50 py-12">
      <div className="mx-auto max-w-2xl px-4">
        <h1 className="font-mono text-2xl font-bold text-slate-900">Account Settings</h1>
