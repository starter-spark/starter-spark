...
export async function updateProfile(formData: FormData) {
  ...
  if (!rateLimitResult.success) {
    return { error: rateLimitResult.error || "Too many updates. Please try again later." }
  }
...
export async function deleteAccount() {
  ...
  if (!rateLimitResult.success) {
    return { error: rateLimitResult.error || "Too many attempts. Please try again later." }
  }

  try {
    // Delete the user's profile (cascades will handle related data)
    // Note: RLS policies prevent users from deleting their own profile directly,
    // so we use the admin client
