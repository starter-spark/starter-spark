...
async function signLearnAsset(bucket: string, path: string): Promise<NextResponse> {
...
export async function GET(request: NextRequest) {
  ...
  if (!lessonId) {
    return NextResponse.json({ error: "Invalid lesson asset path" }, { status: 400 })
  }
  ...
  const moduleRow = lesson.module as unknown as {
    ...
    course: { id: string; is_published: boolean | null; product_id: string } | null
  } | null
  ...
  if (!isStaff) {
    if (lesson.is_published !== true || moduleRow?.is_published !== true || courseRow.is_published !== true) {
