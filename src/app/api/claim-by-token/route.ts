...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!license) {
      // Token missing, used, or invalid.
      return NextResponse.json(
        {
          error:
            'Invalid or expired claim link. Please check your email for a valid link.',
        },
        { status: 404 },
      )
    }
    ...
    if (!claimedLicense) {
      // Race condition, claimed between read and update.
      return NextResponse.json(
        {
          error:
            'This kit was just claimed by another user. Please refresh and try again.',
        },
        { status: 409 },
      )
    }
