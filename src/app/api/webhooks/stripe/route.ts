...
async function loadFulfillment(
  sessionId: string,
): Promise<StripeCheckoutFulfillmentRow | null> {
...
async function markFulfillmentFailed(
  ...
  if (error) {
    console.error(
      'Error updating stripe_checkout_fulfillments failure status:',
      error,
    )
  }
}
...
async function markFulfillmentCompleted(sessionId: string): Promise<void> {
  ...
  if (error) {
    console.error(
      'Error updating stripe_checkout_fulfillments completed status:',
      error,
    )
  }
}
...
async function claimOnce(
  sessionId: string,
  field: 'stock_decremented_at' | 'email_sent_at',
): Promise<boolean> {
...
export async function POST(request: Request) {
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      if (!fulfillment) {
        return NextResponse.json(
          { error: 'Unable to start fulfillment' },
          { status: 500 },
        )
      }
      ...
      try {
        ...
        await markFulfillmentFailed(
          session.id,
          `Stripe line items error: ${message}`,
        )
        return NextResponse.json(
      ...
      // Load existing licenses for this session (legacy rows may lack purchase_item_ref).
      const {
        data: existingSessionLicenses,
        error: existingSessionLicensesError,
      } = await supabaseAdmin
        .from('licenses')
      ...
      if (existingSessionLicensesError) {
        console.error(
          'Error checking existing session licenses:',
          existingSessionLicensesError,
        )
        await markFulfillmentFailed(
          session.id,
          'Failed to check existing licenses',
        )
        return NextResponse.json(
          { error: 'Failed to check existing licenses' },
          { status: 500 },
        )
      }
      ...
      if (
        ...
        console.log(
          `Session ${session.id} already has licenses (legacy), skipping`,
        )
        await markFulfillmentCompleted(session.id)
        return NextResponse.json({
          received: true,
      ...
      // Licenses start as pending and are claimed in workshop.
      ...
      for (let round = 0; round < maxInsertRounds; round++) {
        ...
        if (!isUniqueViolation) {
          ...
          await markFulfillmentFailed(
            session.id,
            `License insert error: ${insertError.message}`,
          )
          return NextResponse.json(
        ...
        console.warn(
          `Unique violation inserting licenses (round ${round + 1}/${maxInsertRounds}); retrying`,
        )
      ...
      if (!insertSucceeded) {
        console.error(
          `Failed to insert licenses for session ${session.id} after ${maxInsertRounds} attempts`,
        )
        await markFulfillmentFailed(
          session.id,
          'Failed to insert licenses after retries',
        )
        return NextResponse.json(
          { error: 'Failed to create licenses' },
          { status: 500 },
      ...
      const { data: allSessionLicenses, error: allSessionLicensesError } =
        await supabaseAdmin
          .from('licenses')
          .select('id,code,product_id,claim_token,purchase_item_ref')
          .eq('stripe_session_id', session.id)
      ...
      if (allSessionLicensesError || !allSessionLicenses) {
        console.error(
          'Error fetching session licenses:',
          allSessionLicensesError,
        )
        await markFulfillmentFailed(
          session.id,
          'Failed to fetch created licenses',
        )
        return NextResponse.json(
          { error: 'Failed to fetch created licenses' },
          { status: 500 },
        )
      }
      ...
      console.log(
        `Session ${session.id}: ${createdOrExisting.length} total licenses (created+existing)`,
      )

      // Decrement stock for tracked inventory.
      const shouldDecrement = await claimOnce(
        session.id,
      ...
      if (shouldDecrement) {
        ...
        for (const [slug, quantity] of quantityBySlug.entries()) {
          ...
          if (product.track_inventory && product.stock_quantity !== null) {
            const { data: updatedStock, error: stockError } =
