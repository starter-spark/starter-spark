...
function getParamString(params: Record<string, unknown> | undefined, key: string, fallback: string): string {
...
// Component palette
const circuitPalette: { type: CircuitComponentType; label: string; icon: React.ReactNode }[] = [
  { type: "arduino", label: "Arduino", icon: <Cpu className="w-4 h-4" /> },
  { type: "led", label: "LED", icon: <Lightbulb className="w-4 h-4" /> },
  { type: "resistor", label: "Resistor", icon: <span className="font-mono text-xs">Ω</span> },
  { type: "capacitor", label: "Capacitor", icon: <span className="font-mono text-xs">||</span> },
  ...
  { type: "ground", label: "GND (-)", icon: <span className="font-mono text-xs">⏚</span> },
  { type: "breadboard", label: "Breadboard", icon: <CircuitBoard className="w-4 h-4" /> },
  { type: "wire_junction", label: "Junction", icon: <span className="font-mono text-xs">●</span> },
]
...
function ArduinoNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative bg-cyan-600 text-white rounded-lg p-3 min-w-[120px] border-2 border-cyan-700 shadow-md">
      ...
      <Handle type="target" position={Position.Top} id="top" className="!bg-yellow-400 !w-3 !h-3" />
      <Handle type="source" position={Position.Bottom} id="bottom" className="!bg-yellow-400 !w-3 !h-3" />
...
function LEDNode({ data }: { data: CircuitNodeData }) {
  ...
  const colorClass = color === "green" ? "bg-green-500" : color === "blue" ? "bg-blue-500" : color === "yellow" ? "bg-yellow-400" : "bg-red-500"

  return (
    <div className="relative flex flex-col items-center">
      <Handle type="target" position={Position.Left} className="!bg-slate-400 !w-2 !h-2" />
      <Handle type="source" position={Position.Right} className="!bg-slate-400 !w-2 !h-2" />
      <div className={cn("w-6 h-6 rounded-full border-2 border-slate-400 shadow-lg", colorClass)} />
      <div className="text-[10px] mt-1 font-mono text-slate-600">{data.label}</div>
    </div>
  )
}
...
function ResistorNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div className="text-[10px] mt-1 font-mono text-slate-600">{data.label} ({getParamString(data.params, "value", "220")}Ω)</div>
    </div>
  )
}
...
function CapacitorNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div className="text-[10px] mt-1 font-mono text-slate-600">{data.label} ({getParamString(data.params, "value", "100")}μF)</div>
    </div>
  )
}
...
function ServoNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative bg-blue-100 border-2 border-blue-400 rounded p-2">
      ...
      <Handle type="target" position={Position.Left} id="vcc" style={{ top: "30%" }} className="!bg-red-500 !w-2 !h-2" />
      <Handle type="target" position={Position.Left} id="gnd" style={{ top: "70%" }} className="!bg-black !w-2 !h-2" />
      <div className="flex items-center gap-2">
...
function SensorNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative bg-green-100 border-2 border-green-400 rounded p-2">
      <Handle type="source" position={Position.Right} className="!bg-yellow-500 !w-2 !h-2" />
      <Handle type="target" position={Position.Left} id="vcc" className="!bg-red-500 !w-2 !h-2" />
      <Handle type="target" position={Position.Left} id="gnd" style={{ top: "70%" }} className="!bg-black !w-2 !h-2" />
      <div className="flex items-center gap-2">
...
function PowerNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative flex flex-col items-center">
      ...
      <div className="text-[10px] mt-1 font-mono text-red-600">{data.label}</div>
    </div>
  )
}
...
function BreadboardNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative bg-white border-2 border-slate-300 rounded p-2 min-w-[200px]">
      <Handle type="target" position={Position.Top} id="power" className="!bg-red-500 !w-2 !h-2" />
      <Handle type="target" position={Position.Top} id="ground" style={{ left: "60%" }} className="!bg-black !w-2 !h-2" />
...
function WireJunctionNode({ data }: { data: CircuitNodeData }) {
  return (
    <div className="relative">
      ...
      <Handle type="target" position={Position.Top} id="top" className="!bg-slate-600 !w-2 !h-2" />
      <Handle type="source" position={Position.Bottom} id="bottom" className="!bg-slate-600 !w-2 !h-2" />
      <div className="w-3 h-3 rounded-full bg-slate-600" />
      {data.label !== "Junction" && (
        <div className="text-[8px] absolute -bottom-3 left-1/2 -translate-x-1/2 font-mono text-slate-500">{data.label}</div>
      )}
    </div>
  )
}
...
export function CircuitDiagramEditor({
  ...
  return (
    <div className={cn("rounded border border-slate-200 bg-white overflow-hidden", className)}>
      <div className="grid lg:grid-cols-[200px_1fr_280px]">
        ...
        {!readOnly && (
          <div className="border-b lg:border-b-0 lg:border-r border-slate-200 p-4 space-y-4">
            <div>
              <p className="font-mono text-sm font-semibold text-slate-900">Components</p>
              <p className="text-xs text-slate-500">Drag components to the canvas</p>
            </div>
        ...
        {!readOnly && (
          <div className="p-4 space-y-4">
            <div className="flex items-center justify-between">
              <p className="font-mono text-sm font-semibold text-slate-900">Properties</p>
              <div className="flex items-center gap-1">
                <Button
                  type="button"
                  variant="ghost"
            ...
            <div className="space-y-2">
              <p className="font-mono text-xs font-semibold text-slate-700">Tips</p>
...
function ComponentParamsEditor({
  ...
  return (
    <p className="text-xs text-slate-500">No editable parameters.</p>
  )
}
