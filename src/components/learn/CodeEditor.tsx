...
import { Check, CheckCircle2, Copy, Download, Moon, RotateCcw, Sun, XCircle } from "lucide-react"
...
function codeMirrorLanguageExtension(language: string): Extension | null {
  ...
  if (lang === "cpp" || lang === "c++" || lang === "c" || lang === "arduino" || lang === "ino") {
    return cpp()
  }
...
const arduinoCompletions: Completion[] = [
  { label: "setup", type: "function", detail: "void setup()", apply: "void setup() {\n  \n}\n" },
  { label: "loop", type: "function", detail: "void loop()", apply: "void loop() {\n  \n}\n" },
  { label: "pinMode", type: "function", detail: "pinMode(pin, mode)" },
  { label: "digitalWrite", type: "function", detail: "digitalWrite(pin, value)" },
  ...
  { label: "Serial.println", type: "function", detail: "Serial.println(value)" },
...
export function CodeEditor({
  ...
  return (
    <div className={cn("rounded border border-slate-200 overflow-hidden", className)}>
      <div className="flex flex-wrap items-center justify-between gap-2 px-4 py-2 bg-slate-100 border-b border-slate-200">
        <div className="flex items-center gap-2 min-w-0">
          {filename && (
            <span className="text-sm font-mono text-slate-600 truncate">{filename}</span>
          )}
                ...
                const className =
                  op.type === "insert"
                    ...
                    : op.type === "delete"
                      ...
                      : "text-slate-700"
                return (
                  <span key={idx} className={cn("block whitespace-pre", className)}>
                    {prefix}
                    {op.line}
                  </span>
                )
