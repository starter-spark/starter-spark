...
test.describe('Community Page - Question Listing', () => {
  ...
  test('should display filter options', async ({ page }) => {
    ...
    const filterElements = page.locator(
      '[role="combobox"], select, [data-filter]',
    )
...
test.describe('Community Page - Question Detail', () => {
  test('should navigate to question detail page', async ({ page }) => {
    ...
    const questionLinks = page.locator(
      'article a[href^="/community/"]:not([href="/community/new"])',
    )
    const count = await questionLinks.count()
  ...
  test('should display answer section', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const hasAnswerSection = await answersText
        .first()
        .isVisible()
        .catch(() => false)
  ...
  test('should display author information', async ({ page }) => {
    ...
    if (count > 0) {
      ...
      const hasAuthor = await authorText
        .first()
        .isVisible()
        .catch(() => false)
...
test.describe('Community Page - New Question', () => {
  ...
  test('should show sign in prompt for unauthenticated users', async ({
    page,
  }) => {
    // Clear auth
    await page.goto('/')
    ...
    const hasSignIn = await signInText
      .first()
      .isVisible()
      .catch(() => false)
...
test.describe('Community Page - Events Toggle', () => {
  test('past events toggle should work on events page', async ({ page }) => {
    ...
    const pastEventsToggle = page.getByRole('button', {
      name: /past events|show past/i,
    })
